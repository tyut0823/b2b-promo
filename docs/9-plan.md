...
| 0.33 | 2026-08-21 | 사용자 리포트로 발견된 UI 이슈 수정: 샘플 목록/상세/관리, 내 신청 내역 화면에서 신청 기간이 `2026-08-21T00:00:00.000Z` 같은 원본 ISO 타임스탬프로 노출되던 문제(0.23, 0.32 변경 이력에 이미 기록된 기존 이슈)를 마무리. `shared/formatDate.ts`(문자열 앞 10자, `YYYY-MM-DD`만 사용) 추가하고 4개 화면(SampleListPage/SampleDetailPage/SampleAdminListPage/MyApplicationsPage)에 적용. 테스트 62개 통과, 빌드 정상 |
| 0.34 | 2026-08-21 | (도메인 정의서/PRD에는 없는 요청 기반 부가 기능) 로그인 시 "룰렛" 기능 추가: 거래처 담당자가 로그인하면 하루 1회 1~3 중 랜덤 숫자가 뽑히는 룰렛 모달이 뜨고, 그 숫자만큼만 그날 샘플을 신청할 수 있다. `stores/rouletteStore.ts`(zustand+persist, 사용자·날짜별 저장, 하루 1회만 재추첨)와 `features/roulette/RouletteModal.tsx`(CSS 애니메이션, 외부 라이브러리 없음) 신규 추가, `BuyerLayout`에 마운트. 제한은 프론트엔드에서만 강제(백엔드/DB 변경 없음) — `useApply` 성공 시 잔여 개수 차감, `SampleDetailPage`/`MyApplicationsPage`의 신청·재신청 버튼을 잔여 개수 0이면 숨김/비활성화. 테스트 70개 통과(신규 6개), 빌드 정상, 실제 브라우저로 룰렛 노출→하루 1회 제한→신청 개수 소진 후 버튼 숨김까지 확인 |
| 0.35 | 2026-08-21 | 사용자 요청으로 룰렛을 실제 회전판 UI로 교체: `RouletteWheel.tsx`(SVG 3분할 파이, 각 값이 상단 고정 포인터에 오도록 각도 계산)를 신규 분리하고, `RouletteModal.tsx`를 숫자 카드 즉시 표시 방식에서 "계속 회전 → STOP 클릭 시 감속하며 해당 값에 정지"하는 requestAnimationFrame 기반 애니메이션으로 교체(외부 라이브러리 없음). `BuyerLayout` nav 우측에 "오늘 신청 가능한 샘플 개수 : n개" 배지를 추가하고 클릭 시 `RouletteResultModal`(같은 휠을 오늘 뽑은 결과 각도로 정지된 채 재표시)을 띄움. 구현 중 STOP 클릭 시 스핀 루프의 useEffect cleanup이 방금 새로 예약된 정지 애니메이션의 rAF를 취소해버리는 실제 레이스 버그를 발견해 ref를 스핀용/정지용으로 분리해 수정(테스트로 재현·검증). 테스트 73개 통과, 빌드 정상, 실제 브라우저로 회전→STOP→정지 각도가 결과 문구와 정확히 일치→nav 배지 갱신→배지 클릭 시 결과 재표시까지 확인 |
| 0.36 | 2026-08-21 | 그동안의 구현 변경사항을 문서에 소급 반영: `1-domain-definition.md`에 로그아웃 유스케이스(4-9)와 6-1절 "일일 신청 가능 개수 룰렛"(부가 기능, 원 요구사항 밖임을 명시) 추가, `3-PRD.md`에 로그아웃/룰렛 기능 및 Supabase Storage·Vercel 배포를 6절 기술 스택에 추가, `4-user-scenario.md`에 로그인 시 룰렛 흐름·신청 개수 초과 예외·로그아웃 시나리오(4.2) 추가, `7-wireframe.md`에 모든 네비게이션의 로그아웃 링크와 오늘 신청 가능 개수 배지, 마이페이지 "현재 비밀번호" 필드, 11절 룰렛 화면 추가, `6-arch-diagram.md`에 Vercel/Supabase(DB+Storage) 반영한 시스템 구성도와 roulette 컴포넌트·rouletteStore 추가, `5-project-principle.md`에 Supabase Storage 관련 백엔드 파일·환경변수와 프론트 roulette 폴더·rouletteStore를 구조도에 반영하고 "authStore 하나로 충분" 문구를 스토어 2개 체제에 맞게 수정, `2-usecase.md`에 로그아웃 UC 추가, `10-style.md`에 룰렛 회전판이 기존 색상 토큰을 재사용함을 명시. 코드 변경 없음(문서만 갱신) |
